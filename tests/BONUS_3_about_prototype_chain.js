describe("About Prototype Chain (topics/BONUS_03_about_prototype_chain.js)", function () {
  ...
  it("Is there an 'a' and 'b' own property on child?", function () {
    expect(true).toBe(child.hasOwnProperty("a"), "child.hasOwnProperty('a')?");
    expect(false).toBe(child.hasOwnProperty("b"), "child.hasOwnProperty('b')?");
  });

  it("Is there an 'a' and 'b' property on child?", function () {
    expect(1).toBe(child.a, "what is 'a' value?");
    expect(2).toBe(child.b, "what is 'b' value?");
  });

  it("If 'b' was removed, whats b value?", function () {
    delete child.b;
    expect(3).toBe(child.b, "what is 'b' value now?");
  });

  it("Is there a 'c' own property on child?", function () {
    expect(false).toBe(child.hasOwnProperty("c"), "child.hasOwnProperty('c')?");
  });
  ...
  it("Is there a 'c' property on child?", function () {
    expect(4).toBe(child.c, "what is the value of child.c?");
  });
  ...
  it("Is there an 'd' property on child?", function () {
    expect(undefined).toBe(child.d, "what is the value of child.d?");
  });
});
